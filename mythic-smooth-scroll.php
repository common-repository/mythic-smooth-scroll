...
defined('ABSPATH') or die(__("Please don't access me in this way. I feel invaded.", 'mythic_ss'));

function mythic_ss_register_settings() {
   add_option( 'mythic_ss_header_id', '');
   add_option( 'mythic_ss_offset_amount', '50');
    
   register_setting( 'mythic_ss_options', 'mythic_ss_header_id', array( 'type' => 'string', 'sanitize_callback' => 'sanitize_text_field' ));
   register_setting( 'mythic_ss_options', 'mythic_ss_offset_amount', array( 'type' => 'string', 'sanitize_callback' => 'sanitize_text_field' ));
}
...
?>
    ...
    <div>
        <h1><?php _e('Mythic Smooth Scroll', 'mythic_ss'); ?></h1>
        <form method="post" action="options.php">
            <?php
                settings_fields( 'mythic_ss_options' );
                do_settings_sections( 'mythic_ss_options' );
            ?>
            <label for="mythic_ss_header_id"><strong><?php _e("Header ID (Leave blank if your header is not fixed)", 'mythic_ss'); ?></strong></label><br>
            <a href="#"><?php _e('Not sure how to find this?', 'mythic_ss'); ?></a><br>
            <input type="text" id="mythic_ss_header_id" name="mythic_ss_header_id" value="<?php echo get_option('mythic_ss_header_id'); ?>" /><br>
            <br>
            <label for="mythic_ss_offset_amount"><strong><?php _e("Scroll Offset Amount (Default is 50). Will be added to fixed header's height.", 'mythic_ss'); ?></strong></label><br>
